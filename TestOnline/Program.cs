...
namespace TestOnline
{
    class Program
    {
        ...
        static void MongoInsertFromDnsrecords()
        {
            ...
            long totalCount = long.Parse(MySQLHelper.Query("SELECT count(1) from zones  where id>3035157 and NSState=1 and State=0").Tables[0].Rows[0][0].ToString());
            Console.WriteLine("{0} rows data", totalCount);
            long zoneidx = 0;
            do
            {
                DataSet ds = MySQLHelper.Query("select a.zone,a.host,a.type,a.data,a.ttl,a.view,a.mx_priority,a.userid,a.id from (SELECT DISTINCT zone from zones where id>3035157 and NSState=1 and State=0 limit " + zoneidx.ToString() + "," + PageSize.ToString() + ")as t inner join dnsrecords as a on t.zone=a.Zone");
                long q = watch.ElapsedMilliseconds;
                DataTable dt = ds.Tables[0];
                long count = dt.Rows.Count;
                Console.WriteLine("query use time：" + q.ToString() + ";" + count + " rows data");
        ...
        static void MongoInsertFromAuthorities()
        {
            System.Diagnostics.Stopwatch watch = new System.Diagnostics.Stopwatch();
            watch.Start();//开始计时     
            DataTable idTab = MySQLHelper.Query("SELECT min(id),max(id) from zones where id>3035157 and NSState=1 and State=0").Tables[0];
            long minID = long.Parse(idTab.Rows[0][0].ToString());
            long maxID = long.Parse(idTab.Rows[0][1].ToString());
            long tempID = minID - 1;
            do
            {
                DataSet ds = MySQLHelper.Query("SELECT a.Zone,Host,Data,Type,TTL,Mbox,Serial,Refresh,Retry,Expire,Minimum,t.userid,a.id FROM (SELECT DISTINCT zone,userid from (select * from  zones where id>3035157 and NSState=1 and State=0) as tab where tab.id>" + tempID.ToString() + " and tab.id<=" + (tempID + PageSize).ToString() + ")as t inner join  authorities as a on t.zone=a.Zone");
                long q = watch.ElapsedMilliseconds;
                DataTable dt = ds.Tables[0];
        ...
        static List<AuthoritiesSimple> Row2Authorities(List<DataRow> drl)
        {
            ...
            for (int i = 0; i < drl.Count; i++)
            {
                ...
                if (d.type == "SOA")
                    d.rdata = "ns1.shelaugh.com." + " " + drl[i + 1][5].ToString() + " " + drl[i + 1][6].ToString() + " " + drl[i + 1][7].ToString() + " " + drl[i + 1][8].ToString() + " " + drl[i + 1][9].ToString() + " " + drl[i + 1][10].ToString();
                else
                {
                    if (drl[i][2].ToString().Contains("s1.xundns")|| drl[i][2].ToString().Contains("vip1"))
                        d.rdata = "ns1.shelaugh.com.";
                    if (drl[i][2].ToString().Contains("s2.xundns")|| drl[i][2].ToString().Contains("vip2"))
                        d.rdata = "ns2.shelaugh.com.";
                }
                if (drl[i][2].ToString().Contains("nsblack"))
                {
                    var c = StringHelper.CalculateMD5Hash(d.domain).ToLower().Substring(0, 1);
                    MongoHelper.Delete<ZonesSimple>("zones", new Document("domain", d.domain));
                    MongoHelper.Delete<ZonesSimple>(c, new Document("domain", d.domain));
                }
                else
                {
            ...
            try
            {
                ...
                foreach (string c in collection)
                {
                    ...
                    foreach (ZonesSimple z in zl)
                    {
                        try
                        {
                            List<DnsRescordsSimple> dl = MongoHelper.GetList<DnsRescordsSimple>(c, new Document("domain", z.domain));
                            if (dl.Count == 0)
                            {
                                MongoHelper.Delete<ZonesSimple>("zones", new Document("domain", z.domain).Add("rrcol", c));
                                zcount++;
                            }
                            else
                            {
                                ...
                                if (soalist.Count == 0 || nslist.Count == 0)
                                {
                                    MongoHelper.Delete<ZonesSimple>("zones", new Document("domain", z.domain).Add("rrcol", c));
                                    zcount++;
                                    MongoHelper.Delete<DnsRescordsSimple>(c, new Document("domain", z.domain));
                                    if (soalist.Count == 0)
                                        Console.WriteLine(z.domain + " no SOA data");
                                    if (nslist.Count == 0)
                                        Console.WriteLine(z.domain + " no NS data");
                                }
                            }
                            dl.Clear();
                        }
