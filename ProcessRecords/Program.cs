...
namespace ProcessRecords
{
    class Program
    {
                            ...
                            if (!processZone.Contains(zone))
                            {
                                ...
                                if (content.Contains(". async without SOA"))
                                {
                                    ...
                                    if (aCount == 0)
                                    {
                                        ...
                                        if (soa.Count == 1)
                                        {
                                            categoriesA.InsertMany(Rows2Authorities(aListtemp));
                                        }
                                        else
                                        {
                                    ...
                                    else
                                    {
                                        ...
                                        if (dtZ.Rows.Count == 1)
                                        {
                                            ...
                                            if (aListInsert.Count > 0)
                                            {
                                                ...
                                                categoriesA.InsertMany(Rows2Authorities(aListInsert));
                                            }
                                        }
                                        else
                                        {
        ...
        static void DoAction3(string path)
        {
            try
            {
                ...
                foreach (string file in files)
                {
                    ...
                    IMongoCollection<ZonesSimple> categoriesZ = db.GetCollection<ZonesSimple>("zones");
                    List<string> processZone = new List<string>();
                    string sql = @"SELECT z.id,z.zone,z.userid,CASE when d.maxfensheng<1 then 0 else 1 end as level,z.nsstate,z.active,z.forcestop,z.rzone from zones as z left join domainlevel as d on z.DomainLevel=d.levelvalue where z.zone='{0}';
                                   select a.id,a.zone,host,data,type,ttl,mbox,serial,refresh,retry,expire,minimum ,a.zoneid,z.userid from authorities as a left join zones as z on z.id=a.zoneid where a.zone='{0}';
                                   select d.id,d.zoneid,d.zone,d.host,d.type,d.data,d.ttl,d.view,d.mx_priority,d.userid,d.active from dnsrecords as d where d.zone='{0}'";
                    while ((content = sr.ReadLine()) != null)
                    {
                        ...
                        if (!processZone.Contains(zone))
                        {
                            ...
                            var zCount = categoriesZ.Find(Builders<ZonesSimple>.Filter.Eq("domain", zone + ".")).ToList<ZonesSimple>().Count;
                            if (zCount == 0)
                            {
                                DataSet ds = MySQLHelper.Query(string.Format(sql, zone));
                                ...
                                List<zones> zList = DtToList<zones>.ConvertToModel(dtz);
                                ...
                                if (zList.Count == 1)
                                {
                                    ZonesSimple zs = Row2ZoneSimple(zList.First());

                                    categoriesZ.DeleteMany(Builders<ZonesSimple>.Filter.Eq("domain", zs.domain));
                                    categoriesZ.InsertOne(zs);
                                    string rrcol = Utility.StringHelper.CalculateMD5Hash(zone + ".").Substring(0, 1).ToLower();

                                    IMongoCollection<AuthoritiesSimple> categoriesA = db.GetCollection<AuthoritiesSimple>(rrcol);
                                    var builderA = Builders<AuthoritiesSimple>.Filter;
                                    var filterA = builderA.And(builderA.Eq("domain", zone + '.'), builderA.Lt("rid", 0));
                                    categoriesA.DeleteMany(filterA);
                                    categoriesA.InsertMany(Rows2Authorities(aList));

                                    IMongoCollection<DnsRecordsSimple> categoriesD = db.GetCollection<DnsRecordsSimple>(rrcol);
                                    var builderD = Builders<DnsRecordsSimple>.Filter;
                                    var filterD = builderD.And(builderD.Eq("domain", zone + '.'), builderD.Gt("rid", 0));
                                    categoriesD.DeleteMany(filterD);
                                    ...
                                    foreach (dnsrecords dr in dList)
                                    {
                                        if (CheckRecordHost(dr.host, dr.type) && CheckRecordData(dr.data, dr.type, dr.view, dr.host))
                                            ...
                                            LoggerAdvance.AddLog("record " + dr.id + " is wrong", "DoAction3", "");
                                    }
                                    categoriesD.InsertMany(dslist);

                                    LoggerAdvance.AddLog(string.Format("zone {0} Refresh", zs.domain), "DoAction3", "");
                                    Console.WriteLine(string.Format("zone {0} Refresh", zs.domain));
                                }
                                else
                                {
                                    LoggerAdvance.AddLog(string.Format("zone {0} has many data", zone), "DoAction3", "");
                                }
                            }
                        }
                        pcount++;
                        if(pcount>0&&pcount%100==0)
                            Console.WriteLine("processed row " + pcount+"  processed zone " + processZone.Count+" use time "+watch.ElapsedMilliseconds);
                    }
                    Console.WriteLine("end processed row " + pcount + "  processed zone " + processZone.Count + " use time " + watch.ElapsedMilliseconds);
                }
            }
            catch (Exception ex)
            {
        ...
        static AuthoritiesSimple Row2Authorities(authorities dr)
        {
                ...
                d.domain = dr.zone.ToLower() + ".";
                d.name = dr.host.ToLower();
                d.type = dr.type.ToString();
                if (d.type == "SOA")//Mbox,Serial,Refresh,Retry,Expire,Minimum
                    d.rdata = dr.data.ToString() + " " + dr.mbox.ToString() + " " + dr.serial.ToString() + " " + dr.refresh.ToString() + " " + dr.retry.ToString() + " " + dr.expire.ToString() + " " + dr.minimum.ToString();
                else
                    d.rdata = dr.data.ToString();
                d.ttl = int.Parse(dr.ttl.ToString());
                d.userid = Convert.ToInt32(dr.userid);
                d.is_stop = "N";
            return d;
        }
        static List<AuthoritiesSimple> Rows2Authorities(List<authorities> drl)
        {
                        ...
                        if ((i > 0 && i % 100 == 0) || i == zoneslist.Count - 1)
                        {
                            ...
                            foreach (string zone in zoneList)
                            {
                                ...
                                if (count == 0 || count > 5)
                                {
                                    ...
                                    if (dt.Rows.Count > 0)
                                    {
                                        List<AuthoritiesSimple> ASList = Rows2Authorities(DtToList<authorities>.ConvertToModel(dt));
        ...
        static ZonesSimple Row2ZoneSimple(zones tz)
        {
            ...
            if (tz.zone != tz.rzone)
                md5 = Utility.StringHelper.CalculateMD5Hash(tz.rzone + ".").ToLower();
            ...
            z.rdomain = tz.rzone + ".";
            z.loadonstart = tz.domainlevel > 0 ? 1 : 0;
            return z;
        }
        ...
        static void UpdateLoadOnStart()
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if ((count > 0 && count % 100 == 0) || count == dt.Rows.Count)
                {
                    var builder = Builders<ZonesSimple>.Filter;
                    var filter = builder.In("domain", zonelist);
                    var update = Builders<ZonesSimple>.Update.Set("loadonstart", 1);
                    categoriesZ.UpdateMany(filter, update);
                    Console.WriteLine("count= {0} time= {1}", count, watch.ElapsedMilliseconds);
                    zonelist.Clear();
                }
        ...
        static void CheckLoadOnStart() {

            var client = DriverConfiguration.Client;
            var db = client.GetDatabase(DriverConfiguration.DatabaseNamespace.DatabaseName);
            IMongoCollection<ZonesSimple> categoriesZ = db.GetCollection<ZonesSimple>("zones");
            List<ZonesSimple> zslist = categoriesZ.Find(Builders<ZonesSimple>.Filter.Eq("loadonstart", 1)).ToList<ZonesSimple>();
            List<ZonesSimple>[] dlz = new List<ZonesSimple>[16] { new List<ZonesSimple>(), new List<ZonesSimple>(), new List<ZonesSimple>(), new List<ZonesSimple>(), new List<ZonesSimple>(), new List<ZonesSimple>(), new List<ZonesSimple>(), new List<ZonesSimple>(), new List<ZonesSimple>(), new List<ZonesSimple>(), new List<ZonesSimple>(), new List<ZonesSimple>(), new List<ZonesSimple>(), new List<ZonesSimple>(), new List<ZonesSimple>(), new List<ZonesSimple>() };
            Console.WriteLine(zslist.Count);
            foreach (ZonesSimple zs in zslist)
            {
                int idx = Int32.Parse(zs.rrcol, System.Globalization.NumberStyles.HexNumber);
                dlz[idx].Add(zs);
            }
            ...
            foreach (string c in collection) {
                int idx = Int32.Parse(c, System.Globalization.NumberStyles.HexNumber);                
                IMongoCollection<AuthoritiesSimple> categoriesA = db.GetCollection<AuthoritiesSimple>(c);
                var filter = Builders<AuthoritiesSimple>.Filter;
                var builder = filter.And(filter.In("domain",domainlist),filter.Eq("type","SOA"));
                List<AuthoritiesSimple> aslist = categoriesA.Find(builder).ToList<AuthoritiesSimple>();
                foreach (string domain in domainlist) {
                    var count = aslist.FindAll(a => a.domain == domain).Count();
                    if (count == 0)
                    {
                        Console.WriteLine(domain + " has no SOA");
                        LoggerAdvance.AddLog(domain + " has no SOA", "CheckLoadOnStart", "");
                    }
                    if (count > 1)
                    {
                        Console.WriteLine(domain + " has many SOA");
                        LoggerAdvance.AddLog(domain + " has many SOA", "CheckLoadOnStart", "");
                    }
                }
                domainlist.Clear();
            }
        ...
        static void ReloadZones()
        {
            ...
            reStart:
            Console.WriteLine(string.Format("index={0};use time={1}", index, watch.ElapsedMilliseconds));
            do
            {
                DataTable dt = MySQLHelper.Query("SELECT z.id,z.zone,z.userid,CASE when d.maxfensheng<1 then 0 else 1 end as level,z.nsstate,z.active,z.forcestop,z.domainlevel,z.rzone from zones as z left join domainlevel as d on z.DomainLevel=d.levelvalue where z.id between " + (index-100) + " and " + index).Tables[0];
                ...
                var builder = Builders<ZonesSimple>.Filter;
                var filter = builder.In("domain", zoneList);
                var zslist = categories.Find(filter).ToList<ZonesSimple>();
                if (zoneList.Count != zslist.Count)
                {
                    string zoneIDStr = "'";
                    List<string> domainList = new List<string>();
                    List<ZonesSimple> zsListInsert = new List<ZonesSimple>();
                    foreach (string domain in zoneList)
                    {
                        var count = (from z in zslist where z.domain == domain select z).ToList<ZonesSimple>().Count;
                        if (count == 0)
                        {
                            zoneIDStr = zoneIDStr + domain.Substring(0, domain.Length - 1) + "','";
                            domainList.Add(domain);
                            zones z = zonesList.Find(tz => tz.zone == domain.Substring(0, domain.Length - 1));
                            zsListInsert.Add(Row2ZoneSimple(z));
                        }
                    }
                    ...
                    if (zoneIDStr != "'")
                    {
                        zoneIDStr = zoneIDStr.Substring(0, zoneIDStr.Length - 2);
                        dta = MySQLHelper.Query("select a.id,a.zone,host,data,type,ttl,mbox,serial,refresh,retry,expire,minimum ,t.userid,a.zoneid from authorities as a left join zones as t on a.ZoneID=t.id where a.ZoneID in (" + zoneIDStr + ") order by a.zone,a.type").Tables[0];
                        dtd = MySQLHelper.Query("select d.id,d.zoneid,d.zone,d.host,d.type,d.data,d.ttl,d.view,d.mx_priority,d.userid,d.active from dnsrecords as d left join zones as t on d.ZoneID=t.id where d.ZoneID in (" + zoneIDStr + ") order by d.zone").Tables[0];
                    }
                    List<authorities> aListtemp = DtToList<authorities>.ConvertToModel(dta);
                    List<AuthoritiesSimple> aslist = Rows2Authorities(aListtemp);
                    foreach (authorities a in aListtemp)
                    {
                        string rrcol = StringHelper.CalculateMD5Hash(a.zone + ".").ToLower().Substring(0, 1);
                        int idx = Int32.Parse(rrcol, System.Globalization.NumberStyles.HexNumber);
                        dla[idx].Add(Row2Authorities(a));
                    }

                    List<dnsrecords> rListtemp = DtToList<dnsrecords>.ConvertToModel(dtd);
                    foreach (dnsrecords dr in rListtemp)
                    {
                        if (CheckRecordHost(dr.host, dr.type) && CheckRecordData(dr.data, dr.type, dr.view, dr.host))
                        {
                            string rrcol = StringHelper.CalculateMD5Hash(dr.zone + ".").ToLower().Substring(0, 1);
                            int idx = Int32.Parse(rrcol, System.Globalization.NumberStyles.HexNumber);
                            dld[idx].Add(Row2DnsRecord(dr));
                        }
                        else
                        {
                            LoggerAdvance.AddLog(string.Format("record " + dr.id + " is wrong"), "WrongRecord", "");
                        }
                    }
                    try
                    {
                        categories.DeleteMany(Builders<ZonesSimple>.Filter.In("domain", domainList));
                        categories.InsertMany(zsListInsert);
                        foreach (string c in collection)
                        {
                            IMongoCollection<AuthoritiesSimple> categoriesa = db.GetCollection<AuthoritiesSimple>(c);
                            IMongoCollection<DnsRecordsSimple> categoriesd = db.GetCollection<DnsRecordsSimple>(c);
                            int idx = Int32.Parse(c, System.Globalization.NumberStyles.HexNumber);
                            categoriesa.DeleteMany(Builders<AuthoritiesSimple>.Filter.In("domain", domainList));
                            if (dla[idx].Count > 0)
                                categoriesa.InsertMany(dla[idx]);
                            if (dld[idx].Count > 0)
                                categoriesd.InsertMany(dld[idx]);
                        }
                        Console.WriteLine("this slice  " + domainList.Count + "   idx=" + index);
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine("Error");
                        LoggerAdvance.AddLog(string.Format("index={0};use time={1}", index, watch.ElapsedMilliseconds), "ReloadZones", "");
                        LoggerAdvance.AddLog(zoneIDStr, "ReloadZones", "");
                        LoggerAdvance.AddLog(ex.Message, "ReloadZones", "");
                        goto reStart;
                    }
                }
            ...
            } while (index > 803700);//2218351
            Console.WriteLine("End index={0};use time={1}", index, watch.ElapsedMilliseconds);
            watch.Stop();//停止计时
        }
        ...
        /// 收费用户
        /// </summary>
        static void ReloadZones2()
        {

            System.Diagnostics.Stopwatch watch = new System.Diagnostics.Stopwatch();
            watch.Start();//开始计时  

            DataTable dt = MySQLHelper.Query("SELECT z.id,z.zone,z.userid,CASE when d.maxfensheng<1 then 0 else 1 end as level,z.nsstate,z.active,z.forcestop,z.domainlevel,z.rzone from zones as z left join domainlevel as d on z.DomainLevel=d.levelvalue where z.DomainLevel>0").Tables[0];
            ...
            for (int i = 0; i < zonesList.Count; i++)
            {
                ...
                if ((i > 0 && i % 100 == 0) || i == zonesList.Count - 1)
                {
                        ...
                        if (zoneList.Count != zslist.Count)
                        {
                            foreach (string domain in zoneList)
                            {
                                ...
                                if (count == 0)
                                {
                                    ...
                                    DataTable dta = MySQLHelper.Query("select a.id,a.zone,host,data,type,ttl,mbox,serial,refresh,retry,expire,minimum ,t.userid,a.zoneid from authorities as a left join zones as t on a.ZoneID=t.id where a.ZoneID=" + z.id + " order by a.zone,a.type").Tables[0];
                                    List<authorities> aListtemp = DtToList<authorities>.ConvertToModel(dta);
                                    List<AuthoritiesSimple> aslist = Rows2Authorities(aListtemp);
                                    DataTable dtd = MySQLHelper.Query("select d.id,d.zoneid,d.zone,d.host,d.type,d.data,d.ttl,d.view,d.mx_priority,d.userid,d.active from dnsrecords as d left join zones as t on d.ZoneID=t.id where d.ZoneID=" + z.id + " order by d.zone").Tables[0];
                                    List<dnsrecords> rListtemp = DtToList<dnsrecords>.ConvertToModel(dtd);
                                    List<DnsRecordsSimple> dslist = new List<DnsRecordsSimple>();
                                    foreach (dnsrecords dr in rListtemp)
                                    {
                                        ...
                                        {
                                            LoggerAdvance.AddLog(string.Format("record " + dr.id + " is wrong"), "recordwrong", "");
                                        }
                                    }
                                    string collectionname = StringHelper.CalculateMD5Hash(domain).ToLower().Substring(0, 1);
                                    IMongoCollection<AuthoritiesSimple> categoriesa = db.GetCollection<AuthoritiesSimple>(collectionname);
                                    IMongoCollection<DnsRecordsSimple> categoriesd = db.GetCollection<DnsRecordsSimple>(collectionname);

                                    categories.DeleteMany(Builders<ZonesSimple>.Filter.Eq("domain", domain));
                                    categoriesa.DeleteMany(Builders<AuthoritiesSimple>.Filter.Eq("domain", domain));
                                    categories.InsertOne(zs);
                                    if (aslist.Count > 0)
                                        categoriesa.InsertMany(aslist);
                                    if (dslist.Count > 0)
                                        categoriesd.InsertMany(dslist);
                                    Console.WriteLine(domain + " Reload");
                                    LoggerAdvance.AddLog(string.Format("zone  " + domain + " is reload"), "ProcessData", "");
                                }
                            }
                        }
                        Console.WriteLine("psocess " + i);
                        Console.WriteLine("============================================== " + i);
                        zoneList.Clear();
                    ...
                    {
                        Console.WriteLine("Error");
                        LoggerAdvance.AddLog(zoneList + "  " + ex.Message, "ProcessData", "");
                    }
                }
            }
            watch.Stop();//停止计时
